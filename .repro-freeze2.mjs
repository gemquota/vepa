import { createMultiplex, startMultiplex, stepMultiplex } from './src/multiplex/multiplex.js';
...
for (let t = 0; t < 200; t++) {
  ...
  if (t % 20 === 0) {
    ...
    console.log(`tick ${t}: iter ${mx.iteration} counts ${mx.shards.map(s=>s.count).join(',')} alive ${mx.shards.map(s=>s.prevAlive).join(',')} heap ${heap}MB`);
  }
}
