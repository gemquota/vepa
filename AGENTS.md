...
> **Active Target:** **VEPA4 v8.5.0** (legacy label `4.8.5`; HEAD `14028fd`, 2026-08-18; v8.5.0 performance knobs committed + tagged — tunable grid/interaction budget + PERFORMANCE accordion + bench `--scale`/`--knobs`/`--report`; **RRP E·F·A trilogy complete** (E.1 v8.2.0 @ `eae1f58` · F v8.3.0 @ `d70ad1b` · A v8.4.0 @ `1e5eead`); design in `docs/dev/rrp-trilogy/`)
...
> **Audit Hash:** `AGENTS_SYNC_v8.5.0_2026-08-18`
>
...
| **Active version** | **VEPA4 v8.5.0** (legacy label `4.8.5`) — tracked in `CHANGELOG.md` + Conventional Commits `chore(release):` subjects (v8.5.0 committed + tagged `v8.5.0`) |
...
├── style.css
├── package.json            ← v4 manifest (8.5.0 — MUST match changelog top)
├── package-lock.json
...
│
├── tests/                  ← vitest: 75 files / 726 tests (unit + audit + params);
...
| **HEAD** | `14028fd` — `chore(release): v8.5.0 — performance knobs: runtime-tunable grid/interaction budget + PERFORMANCE accordion + bench scale/knobs/report` (tagged `v8.5.0`) |
...
| **Working tree** | clean post-v8.5.0 (only untracked `v4-worktree/` leftover, untouched) |
