...
> **Audit Hash:** `AGENTS_SYNC_v8.12.0_2026-08-19`
>
...
| **Active version** | **VEPA4 v8.12.0** (legacy label `4.8.12`) — tracked in `CHANGELOG.md` + Conventional Commits `chore(release):` subjects (v8.12.0 committed + tagged `v8.12.0`) |
...
**Version alignment (new schema since 2026-08-10):** the product is **VEPA4**; versions use `major.minor.build` (npm-semver-native). `VERSION`, the top section of `CHANGELOG.md` (arrow token), `package.json#version`, and this file **must all read 8.12.0**. Since the 2026-08-10 restructure the root manifest IS the v4 manifest (the legacy v2 root `package.json` was archived with the legacy trees).
...
├── style.css
├── package.json            ← v4 manifest (8.12.0 — MUST match changelog top)
├── package-lock.json
...
| **HEAD** | `69a2b71` — `chore(release): v8.12.0 — Set L "Exotic Matter": EXOTIC field zones + per-particle matter states (antimatter annihilation / dark ghosting / strange conversion / negative-mass anti-gravity), MATTER > EXOTIC knobs — L·M·N build 1 opens the physics frontier trilogy³` (tagged `v8.12.0`) |
| **Previous releases** | `5cbea88` — `chore(release): v8.11.1 — boot with PRIME_DEFAULT starter laws` (v8.11.1 hotfix) · `8a39ce9` — `chore(release): v8.11.0 — Set K "Infrastructure & Energy"` (v8.11.0) · `ce691cb` — `chore(release): v8.10.0 — Set J "Society & Governance"` (v8.10.0) · `2472bac` — `chore(release): v8.9.0 — Set I` (v8.9.0) · `81d19c7` — `feat(ui): full-screen multiplex controls` (v8.1.0) · `4b65316` — `feat(physics): accretion is now a true merger` (v8.0.0) |
| **Release tags** | `v8.12.0`, `v8.11.1`, `v8.11.0`, `v8.10.0`, `v8.9.0`, `v8.8.0`, `v8.7.0`, `v8.6.0`, `v8.5.0`, `v8.4.0`, `v8.3.0`, `v8.2.0`, `v8.1.1`, `v8.1.0`, `v8.0.0` (repo tagging adopted at v8.0.0, 2026-08-18) |
| **Backup branches** | `backup/pre-8120-20260819` (cut before v8.12.0 Set L) · `backup/pre-8111-20260819` (cut before v8.11.1 hotfix) · `backup/pre-master-switch-20260811` (cut before the master-switch attempt) · `backup/pre-archive-restructure-20260810` (cut before the 2026-08-10 restructure) · `backup/pre-multiplex-20260807` · `backup/pre-metrics-20260807` · `backup/pre-perf-20260807` · `backup/pre-cleanup-20260726` · `backup/pre-lpsbs-20260728` · `backup/pre-vepa4-20260801` · `backup/v47-multiplex-f6900f0` (pre-force-push v4.7 multiplex evolution, preserved for reference) |
| **Working tree** | clean post-v8.12.0 (only untracked `v4-worktree/` + changelog snapshots, untouched) |
...
- **Unit/audit:** `vepa4 test` (vitest 3.2.7, `tests/`). **82 files / 798 tests** (verified 2026-08-19 — 792 green; the 6 failures are the pre-existing law-category/audit baseline: 4× `lawCategories.test.js` + `batch_08` TIME_DILATION + `batch_30` TELEPORT — untouched by v8.2.0–v8.12.0). Config: `vitest.config.js` includes `tests/**/*.test.js`, node environment, 15 s timeout. Suite layout: `tests/unit/` (26 files incl. `exoticMatter.test.js`, `drawer.test.js`, `fields.test.js`, `groupRegistry.test.js`, `constructionEconomy.test.js`, `livingWorld.test.js`, `perfKnobs.test.js`), `tests/audit/` (`batch_01-32.test.js` + `params_batch_01-18.test.js` + `paramsHelpers.js`).
...
| **Version drift (fixed 2026-08-06, re-based 2026-08-10)** | `VERSION`/changelog arrow token/`package.json#version` all read 8.12.0 (VEPA4 `major.minor.build`); keep them matched per §10.4 |
