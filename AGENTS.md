...
> **Audit Hash:** `AGENTS_SYNC_v8.15.1_2026-08-19`
>
...
| **Active version** | **VEPA4 v8.15.1** (legacy label `4.8.15`) — tracked in `CHANGELOG.md` + Conventional Commits `chore(release):` subjects (v8.15.1 committed + tagged `v8.15.1`) |
...
**Version alignment (new schema since 2026-08-10):** the product is **VEPA4**; versions use `major.minor.build` (npm-semver-native). `VERSION`, the top section of `CHANGELOG.md` (arrow token), `package.json#version`, and this file **must all read 8.15.1**. Since the 2026-08-10 restructure the root manifest IS the v4 manifest (the legacy v2 root `package.json` was archived with the legacy trees).
...
├── style.css
├── package.json            ← v4 manifest (8.15.1 — MUST match changelog top)
├── package-lock.json
...
| **Version drift (fixed 2026-08-06, re-based 2026-08-10)** | `VERSION`/changelog arrow token/`package.json#version` all read 8.15.1 (VEPA4 `major.minor.build`); keep them matched per §10.4 |
