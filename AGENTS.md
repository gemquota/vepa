...
> **Active Target:** **VEPA4 v8.13.0** (legacy label `4.8.13`; HEAD `d64be99`, 2026-08-19; v8.13.0 committed + tagged — Set M "Relativity" (L·M·N build 2): mass-warped CURVATURE field + INFO lensing + velocity time dilation + E=mc²; **RRP trilogies** (E·F·A: E.1 v8.2.0 @ `eae1f58` · F v8.3.0 @ `d70ad1b` · A v8.4.0 @ `1e5eead`; D·G·H: D v8.6.0 · G v8.7.0 · H v8.8.0; I·J·K: I v8.9.0 · J v8.10.0 · K v8.11.0 — **complete**; L·M·N: L v8.12.0 · M v8.13.0 — N v8.14.0 next); designs in `docs/dev/rrp-trilogy/` + `docs/dev/rrp-trilogy-2/` + `docs/dev/rrp-trilogy-3/` + `docs/dev/rrp-trilogy-4/5/6/` (O·P·Q + R·S·T designs locked — the full 18-set lifecycle))
...
> **Audit Hash:** `AGENTS_SYNC_v8.13.0_2026-08-19`
>
...
| **Active version** | **VEPA4 v8.13.0** (legacy label `4.8.13`) — tracked in `CHANGELOG.md` + Conventional Commits `chore(release):` subjects (v8.13.0 committed + tagged `v8.13.0`) |
...
**Version alignment (new schema since 2026-08-10):** the product is **VEPA4**; versions use `major.minor.build` (npm-semver-native). `VERSION`, the top section of `CHANGELOG.md` (arrow token), `package.json#version`, and this file **must all read 8.13.0**. Since the 2026-08-10 restructure the root manifest IS the v4 manifest (the legacy v2 root `package.json` was archived with the legacy trees).
...
├── style.css
├── package.json            ← v4 manifest (8.13.0 — MUST match changelog top)
├── package-lock.json
...
│
├── tests/                  ← vitest: 81 files / 780 tests (unit + audit + params);
...
- **Unit/audit:** `vepa4 test` (vitest 3.2.7, `tests/`). **83 files / 809 tests** (verified 2026-08-19 — 803 green; the 6 failures are the pre-existing law-category/audit baseline: 4× `lawCategories.test.js` + `batch_08` TIME_DILATION + `batch_30` TELEPORT — untouched by v8.2.0–v8.13.0). Config: `vitest.config.js` includes `tests/**/*.test.js`, node environment, 15 s timeout. Suite layout: `tests/unit/` (27 files incl. `exoticMatter.test.js`, `relativity.test.js`, `drawer.test.js`, `fields.test.js`, `groupRegistry.test.js`, `constructionEconomy.test.js`, `livingWorld.test.js`, `perfKnobs.test.js`), `tests/audit/` (`batch_01-32.test.js` + `params_batch_01-18.test.js` + `paramsHelpers.js`).
...
| **Version drift (fixed 2026-08-06, re-based 2026-08-10)** | `VERSION`/changelog arrow token/`package.json#version` all read 8.13.0 (VEPA4 `major.minor.build`); keep them matched per §10.4 |
