...
> **Active Target:** **VEPA4 v8.8.0** (legacy label `4.8.8`; HEAD `04deadc`, 2026-08-18; v8.8.0 Set H committed + tagged — narrative actor + goal-driven species + milestones (D·G·H trilogy build 3 — complete); **RRP trilogies complete** (E·F·A: E.1 v8.2.0 @ `eae1f58` · F v8.3.0 @ `d70ad1b` · A v8.4.0 @ `1e5eead`; D·G·H: D v8.6.0 · G v8.7.0 · H v8.8.0); designs in `docs/dev/rrp-trilogy/` + `docs/dev/rrp-trilogy-2/`)
...
> **Audit Hash:** `AGENTS_SYNC_v8.8.0_2026-08-18`
>
...
| **Active version** | **VEPA4 v8.8.0** (legacy label `4.8.8`) — tracked in `CHANGELOG.md` + Conventional Commits `chore(release):` subjects (v8.8.0 committed + tagged `v8.8.0`) |
...
├── style.css
├── package.json            ← v4 manifest (8.8.0 — MUST match changelog top)
├── package-lock.json
...
│
├── src/                    ← v4 application source (62 JS files, ~20k LOC)
...
│
├── tests/                  ← vitest: 78 files / 747 tests (unit + audit + params);
...
| **HEAD** | `04deadc` — `chore(release): v8.8.0 — Set H "Agency & Narrative": narrative actor + goal-driven species + milestones (D·G·H build 3 — trilogy complete)` (tagged `v8.8.0`) |
...
| **Release tags** | `v8.8.0`, `v8.7.0`, `v8.6.0`, `v8.5.0`, `v8.4.0`, `v8.3.0`, `v8.2.0`, `v8.1.1`, `v8.1.0`, `v8.0.0` (repo tagging adopted at v8.0.0, 2026-08-18) |
...
| **Working tree** | clean post-v8.8.0 (only untracked `v4-worktree/` leftover, untouched) |
