...
> **Active Target:** **VEPA4 v8.11.1** (legacy label `4.8.12`; HEAD `5cbea88`, 2026-08-19; v8.11.1 committed + tagged — boot fix: the dish is alive on load via PRIME_DEFAULT starter laws; **RRP trilogies** (E·F·A: E.1 v8.2.0 @ `eae1f58` · F v8.3.0 @ `d70ad1b` · A v8.4.0 @ `1e5eead`; D·G·H: D v8.6.0 · G v8.7.0 · H v8.8.0; I·J·K: I v8.9.0 · J v8.10.0 · K v8.11.0 — **complete**); designs in `docs/dev/rrp-trilogy/` + `docs/dev/rrp-trilogy-2/` + `docs/dev/rrp-trilogy-3/` + `docs/dev/rrp-trilogy-4/5/6/` (next trilogy³ — in design))
...
> **Audit Hash:** `AGENTS_SYNC_v8.11.1_2026-08-19`
>
...
| **Active version** | **VEPA4 v8.11.1** (legacy label `4.8.12`) — tracked in `CHANGELOG.md` + Conventional Commits `chore(release):` subjects (v8.11.1 committed + tagged `v8.11.1`) |
...
**Version alignment (new schema since 2026-08-10):** the product is **VEPA4**; versions use `major.minor.build` (npm-semver-native). `VERSION`, the top section of `CHANGELOG.md` (arrow token), `package.json#version`, and this file **must all read 8.11.1**. Since the 2026-08-10 restructure the root manifest IS the v4 manifest (the legacy v2 root `package.json` was archived with the legacy trees).
...
├── style.css
├── package.json            ← v4 manifest (8.11.1 — MUST match changelog top)
├── package-lock.json
...
| **HEAD** | `5cbea88` — `chore(release): v8.11.1 — boot with PRIME_DEFAULT starter laws so the dish is alive immediately (zero-laws hard freeze preserved for blank-canvas mode)` (tagged `v8.11.1`) |
| **Previous releases** | `8a39ce9` — `chore(release): v8.11.0 — Set K "Infrastructure & Energy"` (v8.11.0) · `ce691cb` — `chore(release): v8.10.0 — Set J "Society & Governance"` (v8.10.0) · `2472bac` — `chore(release): v8.9.0 — Set I` (v8.9.0) · `81d19c7` — `feat(ui): full-screen multiplex controls` (v8.1.0) · `4b65316` — `feat(physics): accretion is now a true merger` (v8.0.0) · `b7c624c` — `feat(ui): World States panel` (v7.6.0) |
| **Release tags** | `v8.11.1`, `v8.11.0`, `v8.10.0`, `v8.9.0`, `v8.8.0`, `v8.7.0`, `v8.6.0`, `v8.5.0`, `v8.4.0`, `v8.3.0`, `v8.2.0`, `v8.1.1`, `v8.1.0`, `v8.0.0` (repo tagging adopted at v8.0.0, 2026-08-18) |
...
| **Working tree** | clean post-v8.11.1 (only untracked `v4-worktree/` leftover, untouched) |
...
- **Unit/audit:** `vepa4 test` (vitest 3.2.7, `tests/`). **75 files / 726 tests** (verified 2026-08-18 — 720 green; the 6 failures are the pre-existing law-category/audit baseline: 4× `lawCategories.test.js` + `batch_08` TIME_DILATION + `batch_30` TELEPORT — untouched by v8.2.0–v8.5.0). Config: `vitest.config.js` includes `tests/**/*.test.js`, node environment, 15 s timeout. Suite layout: `tests/unit/` (25 files incl. `drawer.test.js`, `fields.test.js`, `groupRegistry.test.js`, `constructionEconomy.test.js`, `livingWorld.test.js`, `perfKnobs.test.js`), `tests/audit/` (`batch_01-32.test.js` + `params_batch_01-18.test.js` + `paramsHelpers.js`).
...
| **Version drift (fixed 2026-08-06, re-based 2026-08-10)** | `VERSION`/changelog arrow token/`package.json#version` all read 8.11.1 (VEPA4 `major.minor.build`); keep them matched per §10.4 |
