...
> **Audit Hash:** `AGENTS_SYNC_v8.14.0_2026-08-19`
>
...
| **Active version** | **VEPA4 v8.14.0** (legacy label `4.8.14`) — tracked in `CHANGELOG.md` + Conventional Commits `chore(release):` subjects (v8.14.0 committed + tagged `v8.14.0`) |
...
**Version alignment (new schema since 2026-08-10):** the product is **VEPA4**; versions use `major.minor.build` (npm-semver-native). `VERSION`, the top section of `CHANGELOG.md` (arrow token), `package.json#version`, and this file **must all read 8.14.0**. Since the 2026-08-10 restructure the root manifest IS the v4 manifest (the legacy v2 root `package.json` was archived with the legacy trees).
...
├── style.css
├── package.json            ← v4 manifest (8.14.0 — MUST match changelog top)
├── package-lock.json
...
| **HEAD** | `499cbc0` — `chore(release): v8.14.0 — Set N "Quantum Macroscale": deterministic superposition with collapse-on-interaction + macro entanglement (stride 75–76, measure-one-collapses-both) + ENERGY-gated wall tunneling + DNA-gated observer effect, MATTER > QUANTUM knobs — L·M·N build 3 (trilogy complete)` (tagged `v8.14.0`) |
...
| **Version drift (fixed 2026-08-06, re-based 2026-08-10)** | `VERSION`/changelog arrow token/`package.json#version` all read 8.14.0 (VEPA4 `major.minor.build`); keep them matched per §10.4 |
