...
> **Active Target:** **VEPA4 v8.1.1** (legacy label `4.8.1`; HEAD `19520e0`, 2026-08-18; Matter & Union physics + v8.1.0 interface & dual-deploy + v8.1.1 narrative-log hotfix; RRP E·F·A trilogy design in `docs/dev/rrp-trilogy/`, uncommitted)
...
> **Audit Hash:** `AGENTS_SYNC_v8.1.1_2026-08-18`
>
...
| **Active version** | **VEPA4 v8.1.1** (legacy label `4.8.1`) — tracked in `CHANGELOG.md` + Conventional Commits `chore(release):` subjects |
...
**Version alignment (new schema since 2026-08-10):** the product is **VEPA4**; versions use `major.minor.build` (npm-semver-native). `VERSION`, the top section of `CHANGELOG.md` (arrow token), `package.json#version`, and this file **must all read 8.1.1**. Since the 2026-08-10 restructure the root manifest IS the v4 manifest (the legacy v2 root `package.json` was archived with the legacy trees).
...
├── style.css
├── package.json            ← v4 manifest (8.1.1 — MUST match changelog top)
├── package-lock.json
...
| **HEAD** | `19520e0` — `fix(ui): newest-first narrative log with autoscroll + idle-noise gating` (v8.1.1) |
...
| **Release tags** | `v8.1.1`, `v8.1.0`, `v8.0.0` (repo tagging adopted at v8.0.0, 2026-08-18) |
...
| **Working tree** | v8.1.1 clean + untracked `v4-worktree/` leftover untouched; untracked `docs/dev/rrp-trilogy/` design artifacts (intent/traceability/mermaid — commit with the next release per R9.3) |
