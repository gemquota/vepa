...
> **Active Target:** **VEPA4 v8.2.0** (legacy label `4.8.2`; HEAD `eae1f58`, 2026-08-18; v8.2.0 Set E.1 "Matter & Medium" field system committed + tagged; RRP E·F·A trilogy design in `docs/dev/rrp-trilogy/`; next: Set F — Civilizations (F.1 group registry → F.4 analytics))
...
> **Audit Hash:** `AGENTS_SYNC_v8.2.0_2026-08-18`
>
...
| **Active version** | **VEPA4 v8.2.0** (legacy label `4.8.2`) — tracked in `CHANGELOG.md` + Conventional Commits `chore(release):` subjects (in the working tree; not yet committed/tagged) |
...
**Version alignment (new schema since 2026-08-10):** the product is **VEPA4**; versions use `major.minor.build` (npm-semver-native). `VERSION`, the top section of `CHANGELOG.md` (arrow token), `package.json#version`, and this file **must all read 8.2.0**. Since the 2026-08-10 restructure the root manifest IS the v4 manifest (the legacy v2 root `package.json` was archived with the legacy trees).
...
├── style.css
├── package.json            ← v4 manifest (8.2.0 — MUST match changelog top)
├── package-lock.json
...
│   ├── ui/                 ← 14 files: world, law, species, dna, dnaAnalytics,
...
│
├── tests/                  ← vitest: 71 files / 695 tests (unit + audit + params);
...
| **HEAD** | `eae1f58` — `chore(release): v8.2.0 — Set E.1 "Matter & Medium" field system (RRP E·F·A build 1)` |
...
| **Release tags** | `v8.2.0`, `v8.1.1`, `v8.1.0`, `v8.0.0` (repo tagging adopted at v8.0.0, 2026-08-18) |
...
| **Working tree** | v8.2.0 clean (fields system + rrp-trilogy artifacts committed); untracked `v4-worktree/` leftover untouched; Set F (Civilizations) builds in progress |
...
- **Unit/audit:** `vepa4 test` (vitest 3.2.7, `tests/`). **71 files / 695 tests** (verified 2026-08-18 — 689 green; the 6 failures are the pre-existing law-category/audit baseline: 4× `lawCategories.test.js` + `batch_08` TIME_DILATION + `batch_30` TELEPORT — untouched by v8.2.0). Config: `vitest.config.js` includes `tests/**/*.test.js`, node environment, 15 s timeout. Suite layout: `tests/unit/` (20 files incl. `drawer.test.js`, `fields.test.js`), `tests/audit/` (`batch_01-32.test.js` + `params_batch_01-18.test.js` + `paramsHelpers.js`).
...
| **Version drift (fixed 2026-08-06, re-based 2026-08-10)** | `VERSION`/changelog arrow token/`package.json#version` all read 8.2.0 (VEPA4 `major.minor.build`); keep them matched per §10.4 |
