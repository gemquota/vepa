...
> **Active Target:** **VEPA4 v8.11.0** (legacy label `4.8.11`; HEAD `8a39ce9`, 2026-08-18; v8.11.0 Set K committed + tagged — policy vector + alliances/conflicts (I·J·K trilogy build 3 — the literal third trilogy, completing the 3×3 **trilogy³**); **RRP trilogies** (E·F·A: E.1 v8.2.0 @ `eae1f58` · F v8.3.0 @ `d70ad1b` · A v8.4.0 @ `1e5eead`; D·G·H: D v8.6.0 · G v8.7.0 · H v8.8.0; I·J·K: I v8.9.0 · J v8.10.0 · K v8.11.0 — **complete**); designs in `docs/dev/rrp-trilogy/` + `docs/dev/rrp-trilogy-2/` + `docs/dev/rrp-trilogy-3/`)
...
> **Audit Hash:** `AGENTS_SYNC_v8.11.0_2026-08-18`
>
...
| **Active version** | **VEPA4 v8.11.0** (legacy label `4.8.11`) — tracked in `CHANGELOG.md` + Conventional Commits `chore(release):` subjects (v8.11.0 committed + tagged `v8.11.0`) |
...
**Version alignment (new schema since 2026-08-10):** the product is **VEPA4**; versions use `major.minor.build` (npm-semver-native). `VERSION`, the top section of `CHANGELOG.md` (arrow token), `package.json#version`, and this file **must all read 8.5.0**. Since the 2026-08-10 restructure the root manifest IS the v4 manifest (the legacy v2 root `package.json` was archived with the legacy trees).
...
├── style.css
├── package.json            ← v4 manifest (8.11.0 — MUST match changelog top)
├── package-lock.json
...
| **HEAD** | `8a39ce9` — `chore(release): v8.11.0 — Set K "Infrastructure & Energy": field-energy extraction + allied grids + era-progressed mega-structures (I·J·K build 3 — the 3×3 trilogy³ COMPLETE)` (tagged `v8.11.0`) |
...
| **Release tags** | `v8.11.0`, `v8.10.0`, `v8.9.0`, `v8.8.0`, `v8.7.0`, `v8.6.0`, `v8.5.0`, `v8.4.0`, `v8.3.0`, `v8.2.0`, `v8.1.1`, `v8.1.0`, `v8.0.0` (repo tagging adopted at v8.0.0, 2026-08-18) |
...
| **Working tree** | clean post-v8.11.0 (only untracked `v4-worktree/` leftover, untouched) |
...
| **Version drift (fixed 2026-08-06, re-based 2026-08-10)** | `VERSION`/changelog arrow token/`package.json#version` all read 8.5.0 (VEPA4 `major.minor.build`); keep them matched per §10.4 |
