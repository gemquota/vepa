...
> **Active Target:** **VEPA4 v8.1.0** (legacy label `4.8.1`; HEAD `81d19c7`, 2026-08-18; The Living World trilogy — Part 1: Matter & Union physics, plus the v8.1.0 interface & dual-deploy release)
...
> **Audit Hash:** `AGENTS_SYNC_v8.1.0_2026-08-18`
>
...
| **Active version** | **VEPA4 v8.1.0** (legacy label `4.8.1`) — tracked in `CHANGELOG.md` + Conventional Commits `chore(release):` subjects |
...
**Version alignment (new schema since 2026-08-10):** the product is **VEPA4**; versions use `major.minor.build` (npm-semver-native). `VERSION`, the top section of `CHANGELOG.md` (arrow token), `package.json#version`, and this file **must all read 8.1.0**. Since the 2026-08-10 restructure the root manifest IS the v4 manifest (the legacy v2 root `package.json` was archived with the legacy trees).
...
├── style.css
├── package.json            ← v4 manifest (8.1.0 — MUST match changelog top)
├── package-lock.json
...
| **HEAD** | `81d19c7` — `feat(ui): full-screen multiplex controls, law icon size toggle, restore Pages mirror` (v8.1.0) |
| **Previous releases** | `4b65316` — `feat(physics): accretion is now a true merger — one body, combined mass, mass-weighted colour` (v8.0.0) · `b7c624c` — `feat(ui): World States panel + toolbar quick-save/undo + compare overlay` (v7.6.0) · `9e3c8cd` — `fix(save): emit world:paramsRestored on restore` (v7.6.0) · `e1a1c99` — `feat(save): world compare + undo ring` (v7.5.0) · `964b1a3` — `feat(save): world-state save/load engine` (v7.4.0) · `921228e` — `feat(multiplex): per-shard world params` (v7.3.0) |
| **Release tags** | `v8.1.0`, `v8.0.0` (repo tagging adopted at v8.0.0, 2026-08-18) |
...
| **Working tree** | clean at v8.1.0 (untracked `v4-worktree/` leftover untouched) |
