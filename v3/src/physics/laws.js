...
export function applyGlowEffect(lawState, view, base, dt, synergy) {
  ...
  // Brightness boost based on signal strength — used by renderer via ALPHA
  // Also emit small energy pulse
  const energy = view[base + S.ENERGY];
...
// ============================================================================
// 47. PHENOTYPE — Express DNA as visual trait modulation
// ============================================================================
export function applyPhenotype(lawState, view, base, dt, synergy) {
  if (!isSet(lawState, 16)) return;
  // Modulate radius based on energy and synergy
  const energy = view[base + S.ENERGY];
...
// ============================================================================
// 49. ACIDITY — Acidic charge damages unprotected particles
// ============================================================================
...
// ============================================================================
// 50. OXIDATION — Charge imbalance causes structural degradation
// ============================================================================
export function applyOxidationEffect(lawState, view, base, dt, synergy) {
  ...
  // High charge degrades mass over time
  const mass = view[base + S.MASS];
...
// ============================================================================
// 51. ISOMERIZATION — Structural rearrangement changes properties
// ============================================================================
export function applyIsomerization(lawState, view, base, dt, synergy) {
  if (!isSet(lawState, 22)) return;
  const age = view[base + S.AGE];
  if (!Number.isFinite(age)) return;
  // Periodically shift radius and charge
...
// ============================================================================
// 52. CHIRALITY — Handedness affects interaction bias
// ============================================================================
export function applyChirality(lawState, view, iBase, jBase, dx, dy, dz, dist, synergy) {
  if (!isSet(lawState, 23)) return null;
  if (dist < 1) return null;
  // Particles with same-sign POLARITY are repelled slightly
  const polI = view[iBase + S.DNA_CACHE_START + 4]; // POLARITY
  ...
  if ((polI > 0 && polJ > 0) || (polI < 0 && polJ < 0)) {
    const strength = 0.01 * synergy;
    const invDist = 1.0 / dist;
    // Perpendicular force (rotational bias)
    return {
      ax: -dy * invDist * strength,
      ay: dx * invDist * strength,
      az: 0,
...
// ============================================================================
// 53. CRYSTALLIZATION — Particles align into ordered lattice
// ============================================================================
export function applyCrystallization(lawState, view, iBase, jBase, dx, dy, dz, dist, synergy) {
  ...
  // Attraction toward discrete lattice positions
  const gridSize = 8.0;
...
// ============================================================================
// 54. PHASE_RADIATION — Hot particles radiate energy as light
// ============================================================================
...
// ============================================================================
// 55. SUBLIMATION — Solid particles skip liquid phase, turn directly to gas
// ============================================================================
...
// ============================================================================
// 56. TRACK — Predation tracking: chase lower-mass particles
// ============================================================================
export function applyTrackingBehavior(lawState, view, iBase, jBase, dx, dy, dz, dist, synergy) {
  ...
  const predationBias = view[iBase + S.DNA_CACHE_START + 36]; // PREDATION_BIAS
  if (!Number.isFinite(predationBias) || predationBias < 0.1) return null;
  // If prey is lower mass, chase it
  if (massJ < massI * 0.8) {
...
// ============================================================================
// 57. GENOTYPE — DNA mutation from environmental stress
// ============================================================================
export function applyGenotypeMutation(lawState, view, base, dt, synergy) {
  if (!isSet(lawState, 15)) return;
  const mutationRate = view[base + S.DNA_CACHE_START + 12]; // MUTATION
