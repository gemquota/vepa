...
### Architecture — World-aggregate adoption + layering record (P3–P7 roadmap)
- **Multiplex shards own World aggregates (P3)** — `createShard` wraps each shard's buffer/DNA/laws/counters in a `createWorld` view (`shard.world`); `snapshotShard`/`restoreShard` now delegate to `snapshotWorld`/`restoreWorld` (canonical shape: `version`/`worldParams`/`lawState`/`dna`/`particle`/`particleCount`/`speciesCount`/`tick`/`metadata`) plus the shard-local extras (PRNG stream, offspring tally, alive window, source genome). `shardFromSnapshot` (follow-mode metrics) reads the aggregate shape. No cycles, no buffer reallocation.
- **Timeline snapshots are live-region (P6)** — the intelligence controller's timeline recording captured the *entire* capacity buffer per snapshot (MAX_PARTICLES × stride × 4 bytes ≈ 8 MB each); it now captures only `count × stride`, the same live-region discipline as `snapshotWorld`. Scrubs are unchanged semantically (count restored from metadata; dead region beyond count is never read).
- **`docs/ARCHITECTURE.md` (P4/P5/P7)** — the living architecture record fixing the ubiquitous language (World, Law Set, Species, Population, Tick, Lineage, Timeline, Observation, Shard, Fitness), the four bounded contexts (Kernel → Application → Presentation, with Experimentation driving the kernel), the dependency rules, the canonical state-transfer boundary, the remaining coupling findings (§4: constants.js gravity, runtimeConfig singleton, UI receiving domain objects, opt-in worker), and the future-domain/capability horizon (deterministic headless execution, shards-as-worlds evolutionary runner, benchmark-of-shards).
- Verification: multiplex 25/25 (snapshot shape updated), full suite 655/655, `vepa4 syntax` clean, `vepa4 bench` smoke green (500 particles ~3.9 ms/tick, 0 NaN).

### Architecture — the World aggregate (P2 roadmap: the World is the real aggregate)
- **`src/world/world.js`** — the audit's central recommendation, now concrete: a first-class World aggregate owning every piece of simulation state that used to live in scattered module-level variables — `worldParams`, `lawState`, the DNA species table, the particle buffer, population, time and metadata. Shape: `{ version, worldParams, lawState, dna, particle: { buffer, view, isShared, stride, maxParticles }, worldSize, population: { count, speciesCount }, time: { tick }, metadata: { name, seed, createdAt, note } }`. Headless and pure: no module-level mutable state, no DOM, no worker globals — the same world object can be owned by the main thread, a Web Worker, a Node process, or a server.
- **State-transfer boundary** — `snapshotWorld`/`restoreWorld` (deep in-memory copies; particle payload covers only the live region `count × stride`, never the full capacity buffer) and `serializeWorld`/`deserializeWorld` (JSON-safe persistence for presets, import/export, server round-trips). `worldReport` is the World → Observation boundary: a plain population census (alive/dead/NaN, per-species), active law count and clock summary for engines and UI — NaN semantics match the debug overlay convention.
- **Worker protocol elevated** — `physics.worker.js` now defines its state in terms of the aggregate: `GET_STATE` → `snapshotWorld(world, { includeParticles: false })` (caller reads the shared buffer directly), `RESTORE` → `restoreWorld(world, payload)` with `tickCount`→`tick` normalization. Reply field names unchanged.
- **`main.js` now owns one `world` handle** — `particleBuffer`/`particleView`/`lawState`/`dnaBuffer`/`worldParams` module variables eliminated; boot builds `createWorld(...)` and every read/write routes through `world.*` (solver call, chaos paths, spawn caps, debug census, render, multiplex import, param changes). Scalar counters mirror into `world.population`/`world.time.tick` on the restart, import, and per-tick paths.
- Tests: `tests/unit/world.test.js` +12 (composition/defaults/overrides, snapshot↔restore round-trip incl. law 63 high-bit, partial-snapshot restore, live-region-only copy, JSON round-trip, clock advance, report census + NaN flagging). Worker protocol suite still 14/14 incl. byte-identical worker/main parity. Full suite 655/655; `vepa4 bench` smoke green (500 particles, ~5 ms/tick, 0 NaN).

### Architecture — portable solver (P0 roadmap: hidden dependencies eliminated)
- **Explicit simulation context** — `solve()` no longer imports the mutable `runtimeConfig` singleton; it takes a final `simContext` argument whose default is the static, frozen `SIM_CONTEXT_DEFAULTS`. The solver is now a pure function of its inputs — the same world runs identically in the main thread, a Web Worker, a Node process, or a server.
- **`src/physics/simContext.js`** — first-class SimulationContext: `SIM_CONTEXT_DEFAULTS` (baseline), `createSimContext(overrides)`, and `simContextFromRuntimeConfig(runtimeConfig)` for the app boundary. `main.js`, `multiplex.js`, and the benchmark pass live tunables through the factory instead of hand-built literals.
- Tests: `tests/unit/simContext.test.js` +4 (defaults mirror the runtimeConfig baseline; overrides merge without mutating defaults; solve() parity for omitted-vs-explicit context; solver isolation from runtimeConfig mutation). `vepa4 syntax` clean; full suite 612/612.

### Architecture — laws.js extraction begins (P0 roadmap: second God Object)
- **`src/physics/lawsState.js`** — new module owning all module-level mutable law state: the shared particle-buffer reference (`setBuffer`/`buffer_global`), the HISTORY law's 12³ spatial memory field (write/calc/force moved here with their state), the FATE clock, and the shared helpers (`clamp`, `nanGuard`, `readDNA`, `worldParams`). `laws.js` now imports from it instead of declaring the state itself.
- **Thermodynamics migrated** — the 11 legacy thermo laws (`applyHeat`, `applyCold`, `applyMelt`, `applyBoil`, `applyCondense`, `applyDeposit`, `applyExothermic`, `applyHeatTransfer`, `applyThermalJitter`, `applyColdDamping`, `applyConvection`) moved into `lawgroups/thermoLaws.js`.
- **Electromagnetism migrated** — all 14 legacy EM laws (`applyChargeForce` … `applySuperconductivity`) moved into `lawgroups/emLaws.js` (plus `buffer_global`/`readDNA` imports; the group's existing local `clamp`/`nanGuard` cover the rest).
- **`laws.js` is now a barrel** — it re-exports the migrated categories (thermo, em, history) and the remaining 71 legacy laws, keeping the solver's `from './laws.js'` import surface and every test import untouched. No cross-file calls existed between the remaining laws (verified: all 85 remaining exports are independent leaves), so the rest of the migration is mechanical.
- **Extraction completed — all 71 remaining legacy laws migrated out of `laws.js`** into their category modules: physics (10 → `physicsLaws.js`), biology (12 → `biologyLaws.js`), chemistry (14 → `chemistryLaws.js`), thermodynamics (2 → `thermoLaws.js`), metaphysics (14 → `metaLaws.js`), information (17 → `infoLaws.js`), quantum (2 → `quantumLaws.js`). Per-group helpers moved with their laws (`readSpeciesDNAParam`/`writeSpeciesDNAParam`/`hslToRgb`/`ENERGY_CHANNELS` → biology, `channelMatch` → info, `SINGULARITY_MASS`/`breakBondPair` → physics); `BOND_SLOTS` (shared by physics BOND + chemistry POLYMER/ISOMERIZATION) is now exported from `lawsState.js`.
- **`laws.js` is now a pure barrel (2,129 → 145 lines)** — a stable re-export contract with a comment per law index. `runtimeConfig` and the dead `getDNAFloat` import are gone from the physics layer.
- Tests: `tests/unit/lawsBarrel.test.js` +3 locks the solver's exact law import surface (every name must stay resolvable from the barrel). `vepa4 syntax` clean; full suite 629/629; `vepa4 bench` smoke green.

### Architecture — population subsystem (P1 roadmap: main.js God object)
- **`src/spawn/population.js`** — new module owning the species profiles (`SPECIES_PROFILES`, `EXTRA_SPECIES_COLORS`, `profileColor`, `setDNAFromProfile`) and every spawn path (`createPopulationController` → `spawnSingleParticle` / `spawnDefaultPopulation` / `spawnOffspring`). The controller is a pure function of its inputs: `count` is passed in and the new count returned, so main.js keeps its own counters and no hidden module-level population state exists.
- **`main.js` slimmed 845 → ~590 lines** — the bootstrap file no longer carries the roster/spawn logic; it wires the controller into boot (initial population), the restart path, the SPAWN_RATE feed, and the REPRO offspring drain. Lineage birth tracking is injected via the `onBirth` callback.
- **Bug caught by the new tests**: the initial extraction shadowed the DNA buffer with the loop variable (`d`), writing NaN into every particle's DNA cache — silently disabling REPRO (drive accumulated to NaN). Fixed with a regression assertion that all 42 DNA cache loci stay finite.
- Tests: `tests/unit/population.test.js` +8 (profile shape/colour/quantization, initial spawn field init + finite DNA cache, roster/genome preservation, single-particle append + cap, REPRO offspring integration through the solver ring). Full suite 637/637; `vepa4 bench` smoke green.

### Architecture — intelligence orchestration (P1 roadmap: main.js God object)
- **`src/engines/intelligence.js`** — new `createIntelligenceController(env)` owning all five interpretation engines (insight, narrative, lineage, goal, timeline), their per-tick orchestration, goal-adjustment application, timeline scrub/record bus wiring, death-cause classification, and reset semantics. Pure-function controller: particle view, counts, world size, law count, FPS, and runtime config are passed in through getters/callbacks, so main.js keeps its own counters and the module holds no hidden population state.
- **`main.js` slimmed 606 → 481 lines** — the bootstrap file no longer instantiates the engines or carries the `wireGoalEvents`/`computeMetrics`/`updateIntelligence`/`resetIntelligence` block; it wires the controller with getters (`view`/`count`/`worldSize`/`lawCount`/`fps`/`runtimeConfig`) and an `onRestore` callback so timeline scrubs still update `particleCount`/`tick`. Lineage births flow back in through `intelligence.birth()` in the population `onBirth` hook. All engine imports removed from `main.js`.
- Behavior preserved exactly: `sim:metrics` every 30 ticks, timeline snapshots on the 150-tick cadence, `goal:adjusted` → runtime config/insight cfg mutation + `goal:applied`, scrub restores the buffer, count, and tick, and `reset()` clears timeline + insight/goal history.
- Tests: `tests/unit/intelligence.test.js` +6 (birth delegation, 30-tick metrics with live stats, death cause classification starvation/energy-depletion, goal adjustment application, 150-tick snapshot + scrub restore, reset timeline clearing). Full suite 643/643; `vepa4 bench` smoke green.

### Architecture — multiplex shard machinery (P1 roadmap: multiplex.js God object)
- **`src/multiplex/shards.js`** (new) — pure shard factory: `createShard` (independent { buffer, dna, laws, prng } derived from a source sim), `applyVariation` (per-aspect law/DNA/population variation), `spawnShardPopulation` (fresh grid spawn), `spawnShardOffspring` (REPRO drain into the shard buffer), and `snapshotShard`/`restoreShard` (keep-selected anchor round-trip).
- **`src/multiplex/metrics.js`** (new) — pure fitness layer: `FITNESS_METRICS`/`ALIVE_WINDOW_SIZE`/`EXPLORATION_BINS`/`BASE_FITNESS_METRICS`, `countAlive`/`updateShardWindow` (alive-window rolling stats), `computeShardMetrics` (13 raw metrics incl. Shannon diversity, spatial entropy, novelty, stability), `getFitnessReport` (min-max normalization + modes + delta + weighted composite), and `summarizeMultiplex`.
- **`src/multiplex/defaults.js`** (new) — the `MULTIPLEX_DEFAULTS` config surface, shared by the controller and the fitness report without a module cycle.
- **`multiplex.js` 985 → 413 lines** — now only the controller lifecycle (`createMultiplex`/`startMultiplex`/`stopMultiplex`/`iterateMultiplex`/`stepMultiplex`), selection policies (`selectFittestShard`/`selectFollowShard`), the DOM grid (`buildShards`/`mountShards`/`renderMultiplex`/`resizeMultiplex`/`selectShard`), and the import/export surface. It re-exports every name consumers use (main.js, multiplexUI.js, tests) so the public API is unchanged.
- No cycles: multiplex.js → { shards.js, metrics.js, defaults.js }, shards.js → metrics.js, metrics.js → defaults.js. All 23 existing multiplex tests pass unchanged through the barrel; full suite 643/643; `vepa4 bench` smoke green.
