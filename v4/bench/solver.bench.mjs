#!/usr/bin/env node
/**
 * VEPA v4 — Physics solver benchmark
 *
...
function throughputTable() {
  ...
  console.log(`\nVEPA v4 solver benchmark — ${TICKS} ticks (${WARMUP} warmup), laws: ${DEFAULT_LAWS.join(', ')}`);
...
function stressTable() {
  ...
  if (JSON_OUT) return rows;
  console.log(`\nVEPA v4 solver stress — ALL ${LAW_COUNT} laws on, ${TICKS} ticks (${WARMUP} warmup)`);
  console.log('─'.repeat(64));
  for (const r of rows) console.log(formatRow(r));
  return rows;
}
...
function perLawTable() {
  ...
  console.log(`\nVEPA v4 per-law overhead (${count} particles) — default set + law vs baseline`);
  console.log('─'.repeat(64));
