import { DNA_META, DNA_RANGES, HELP_DB, DRONE_COMMENTS } from './constants.js';
    ...
    if (engine.particles) {
        ...
        for (let i = 0; i < engine.worldConfig.count; i++) {
            ...
            if (engine.particles[ptr+13] === 0) {
                ...
                // Track colors with grouping
                const r = Math.round(engine.particles[ptr+14] * 255);
                const g = Math.round(engine.particles[ptr+15] * 255);
                const b = Math.round(engine.particles[ptr+16] * 255);
                
                let found = false;
                for (const group of colorGroups) {
                    const dist = Math.sqrt((r - group.r)**2 + (g - group.g)**2 + (b - group.b)**2);
                    if (dist < COLOR_THRESHOLD) {
                        // Average the color slightly toward the new member
                        const weight = 1 / (group.count + 1);
                        group.r = Math.round(group.r * (1 - weight) + r * weight);
                        group.g = Math.round(group.g * (1 - weight) + g * weight);
                        group.b = Math.round(group.b * (1 - weight) + b * weight);
                        group.count++;
                        found = true;
                        break;
                    }
...
function drawScatterPlot(engine) {
    ...
    for (let i = 0; i < engine.worldConfig.count; i++) {
        const ptr = i * STRIDE;
        if (engine.particles[ptr+13] === 0) {
            const mass = engine.particles[ptr+11];
            const nrg = engine.particles[ptr+22];
            const sIdx = Math.floor(engine.particles[ptr+12]);
