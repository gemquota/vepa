...
export class GoalSystem {
    ...
    applyGoalInfluence() {
        if ((this.engine.complexityLevel || 0) < 2) return; // Lower threshold to 2
        
        const { primary, tension } = this.inferDominantField();

        // Soft biasing of laws
        if (primary.name === "stability") {
            this.engine.laws.dragCoeff = Math.min(0.999, this.engine.laws.dragCoeff + 0.0002);
            this.engine.laws.dt = Math.max(0.5, this.engine.laws.dt - 0.001);
        }
        if (primary.name === "entropy") {
            this.engine.laws.dragCoeff = Math.max(0.98, this.engine.laws.dragCoeff - 0.0002);
            this.engine.laws.dt = Math.min(2.0, this.engine.laws.dt + 0.001);
        }
        if (primary.name === "complexity") {
            this.engine.laws.G = Math.min(0.5, this.engine.laws.G + 0.0001);
        }

        // High tension → chaotic drift of DNA in all species
        if (tension > 4) {
            this.engine.species.forEach(s => {
                const dnaIdx = 3; // Jitter
                s.dna[dnaIdx] = Math.min(0.5, s.dna[dnaIdx] + 0.001);
            });
        }

        // Stability-driven DNA alignment
        if (primary.name === "stability" && tension < 2) {
            this.engine.species.forEach(s => {
                const dnaIdx = 1; // Viscosity
