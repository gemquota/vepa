...
export class InsightEngine {
    ...
    detectFusionDynamics(state) {
        ...
        state.species.forEach((s, i) => {
            const dna = s.dna;
            const fusion = dna[9]; // Fusion
            const fTime = dna[17]; // Fusion Time
            const momentum = dna[16]; // Fusion Momentum
    ...
    detectGravitationalCollapse(state) {
        ...
        state.species.forEach((s, i) => {
            const dna = s.dna;
            const force = dna[0]; // Force
            const mass = dna[7]; // Hidden Mass
    ...
    detectSwarmBehavior(state) {
        ...
        state.species.forEach((s, i) => {
            const dna = s.dna;
            const resp = dna[13]; // Signal Resp
            const strength = dna[19]; // Signal Strength
            const decay = dna[20]; // Signal Decay
    ...
    detectThermalChaos(state) {
        ...
        state.species.forEach((s, i) => {
            const dna = s.dna;
            const jitter = dna[3]; // Jitter
            const viscosity = dna[1]; // Viscosity
    ...
    detectPopulationDynamics(state) {
        ...
        state.species.forEach((s, i) => {
            const dna = s.dna;
            const birth = dna[10]; // Birth Rate
            const death = dna[11]; // Death Rate
