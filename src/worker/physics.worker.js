...
function step(laws, specDNA, world) {
    ...
    for (let i = 0; i < count; i++) {
        const ptr = i * STRIDE;
        // BIOL check for spawning
        if (laws.life && particles[ptr + 13] > 0 && Math.random() < world.spawnRate * 0.05) {
            ...
            particles[ptr+18] = particles[ptr+19] = particles[ptr+20] = 0;
        }
        if (particles[ptr + 13] > 0) continue;
        
        const spId = particles[ptr + 12], dna = specDNA[spId] || specDNA[0];
        
        // GLOW check (handled as signal resp in physics for now)
        if (laws.glow) {
                                ...
                                if (laws.accr) {
                                    ...
                                    if (relMom >= dna.fusionMomentum) {
                                        ...
                                        if (particles[ptr+17] >= dna.fusionTime) {
                                            ...
                                            const v1x = particles[ptr+3], v1y = particles[ptr+4], v1z = particles[ptr+5];
                                            const v2x = particles[oPtr+3], v2y = particles[oPtr+4], v2z = particles[oPtr+5];
                                            
                                            particles[ptr+3] = (v1x*m1 + v2x*m2) / newM;
                                            particles[ptr+4] = (v1y*m1 + v2y*m2) / newM;
                                            particles[ptr+5] = (v1z*m1 + v2z*m2) / newM;
                                            
                                            // Conservation of Angular Momentum (Simplified)
                                            // L = r x p + I*w
                                            // For simplicity, we add the orbital angular momentum to the spin of the new body
                                            const rx = dx, ry = dy, rz = dz;
                                            const p2x = v2x*m2, p2y = v2y*m2, p2z = v2z*m2;
                                            const Lx = ry*p2z - rz*p2y, Ly = rz*p2x - rx*p2z, Lz = rx*p2y - ry*p2x;
                                            
                                            // Combine spins (weighted by mass-radius squared)
                                            const I1 = (2/5) * m1 * m1; // Approximation I = (2/5)MR^2, R=sqrt(M) -> I = (2/5)M^2
                                            const I2 = (2/5) * m2 * m2;
                                            const newI = (2/5) * newM * newM;
